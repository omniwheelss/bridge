...
class DynamicController
{
	...
	**/
	public function handle_screen($do, $user_account_id, $user_type_id, $read_access, $write_access)
	{
		...
		**/
		if ($do == 'add_user' && $clm && $ac) {echo "Seeni";exit;
	...
	**/
	static function func_search_element($field_type, $field_name, $field_value, $manditary, $common_value, $extra) {
		...
		switch ($field_type) {
			...
			case 'date':
				$search_fields = '<input type="text" name="'.$field_name.'" id="'.$field_name.'" value="'.$field_value.'" '.$manditary.' class ="'.$cal.' form-control" '.$extra.' />';
				$calendar_fields = '
